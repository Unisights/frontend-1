// `unisights-frontend/src/pages/Consultant.jsx`
import { useEffect, useState } from 'react';
import Navbar from '../components/Navbar';
import api from '../api/axiosClient'; // baseURL = http://localhost:8080/api/v1
export default function Consultant() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/consult/queue'); // GET /api/v1/consult/queue
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  const decide = async (id, decision) => {
    ...
    try {
      ...
      // reflect change (remove or mark)
      setItems(list => list.map(r => r.app_id === id ? { ...r, status: decision } : r));
    } finally {
      setBusyId(null);
    }
  };
